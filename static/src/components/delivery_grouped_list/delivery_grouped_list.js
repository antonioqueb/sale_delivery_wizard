...
export class DeliveryGroupedList extends Component {
    ...
    async _buildGroups() {
        ...
        try {
            ...
            // 1) Prioridad absoluta: guardar el wizard y leer por ORM.
            //    Esto evita el problema de Odoo 19 con one2many transients
            ...
            // 2) Si por alguna razón no hubo resId, sólo confiar en OWL cuando
            //    las líneas NO-section realmente traigan datos de negocio.
            if (this._recordsLookHydrated(owlLines)) {
    ...
    _recordsLookHydrated(lines) {
        ...
        return dataLines.every((line) => {
            ...
            const hasMeaningfulPayload =
                d.qty_available !== undefined ||
                d.qty_to_deliver !== undefined ||
                d.qty_delivered !== undefined ||
                d.qty_to_return !== undefined ||
    ...
    async _ensureWizardSaved() {
        ...
        try {
            ...
            // OJO: NO usar active_id aquí.
            // active_id es la sale.order, no el wizard.
            this._wizardId = null;
        }
    }
    ...
    async _buildGroupsFromOrm(owlRecords) {
        ...
        if (!lineData.length) {
            // Último fallback: sólo si OWL viene realmente hidratado.
            if (this._recordsLookHydrated(owlRecords)) {
                this._buildGroupsFromRecords(owlRecords);
            } else {
                this.state.groups = [];
            }
            return;
